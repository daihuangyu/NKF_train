...
wav_output_path = '/mnt/users/daihuangyu/AEC_Challenge/NKF_Data_From_AEC_Challenge/single_talk_B12/'
...
#     sig_len = np.random.randint(2000, 14000)
...
def energy_corrector(ref, echo):
    # 回声随机降
    echo = echo/np.random.uniform(low=1.2, high=1.6, size=1)
    # 能量矫正器
    P_echo = np.sum(abs(echo) ** 2) / len(echo)
    P_ref = np.sum(abs(ref) ** 2) / len(ref)
    ref = np.sqrt(P_echo / P_ref) * ref / np.random.uniform(low=1, high=1.2, size=1)
    return ref, echo
...
def data_combine(start, end, need_len, echo_low, echo_high):
    ...
    for i in range(start, end):
        ...
        nearend_speech = np.zeros(16000)
        #nearend_speech = np.clip(nearend_speech,-0.9,0.9)
        # RIRs, _ = sf.read(f"./feature_extract/fast_rir/Generated_RIRs/RIR-{i}.wav") # 需要进行修改，延时小于150ms
        RIR_sample = np.random.randint(20, 180)
