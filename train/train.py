...
#             weight_p += [p]    
    ...
    #optimizer = optim.RAdam([{'params': weight_p, 'weight_decay':1e-5},
    #                  {'params': bias_p, 'weight_decay':0}], 
    #                  lr=config['model'][0]['lr'])
    ...
    for epoch in range(start_ep + 1, epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            loss = model.calloss2(outputs, echo_wav)            
#            weight_p, bias_p = [],[]
#             for name, p in model.named_parameters():
#                 if 'bias' in name:
#                     bias_p += [p]
...
#                                     lr=config['model'][0]['lr']) 
            optimizer.zero_grad() 
            loss.backward()
            torch.nn.utils.clip_grad_norm_(parameters=model.parameters(), max_norm=2, norm_type=2)
            optimizer.step()
            step += i
...
def val(model, device, config):
    ...
    with torch.no_grad():
        for i, data in enumerate(valloader):
            ...
            train_wav, speech_wav, ref_wav, echo_wav  = train_wav.to(device), speech_wav.to(device), ref_wav.to(device), echo_wav.to(device)
            outputs = model(ref_wav, train_wav)
            loss = model.calloss3(outputs, echo_wav)
            if float(loss)  > 100:
                continue
            total_loss += loss.item()
            print(loss)
...
if __name__ == '__main__':
    ...
    configuration = 'train_1024.json5'
