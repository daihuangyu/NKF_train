...
def test_aec(model, device):
    start = time.time()
    model.load_state_dict(torch.load('./model/nkf_res_1024/checkpoint_61_172941001.pth')['model_state_dict'], strict=True)
    #model.load_state_dict(torch.load('./model/baseline/nkf_epoch70.pt'), strict=True)
    model.eval()
    fid = open("test_log_nkf_res1024.txt", "w")
    batch_size = 12
    ...
    with torch.no_grad():
        for i, data in enumerate(testloader):
            ...
            ref_wav, train_wav, speech_wav, s_hat, res_hat, echo_hat = model(ref_wav, train_wav, speech_wav)
            
            ref_wav, train_wav, speech_wav = ref_wav.cpu().numpy(), train_wav.cpu().numpy(), speech_wav.cpu().numpy()
            s_hat, res_hat, echo_hat = s_hat.cpu().numpy(), res_hat.cpu().numpy(), echo_hat.cpu().numpy()
            erle_db_tmp = []
            pesq_score_tmp = []
            for j in range(batch_size):
                pesq_score = PESQ(res_hat[j], speech_wav[j])
                pesq_scores.append(pesq_score)
                pesq_score_tmp.append(pesq_score)
                erle_db = ERLE(res_hat[j], echo_hat[j])
